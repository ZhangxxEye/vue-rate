<template>
  <div class="vux-rater">
    ...
    <a class="vux-rater-box" v-for="i in max" @click="handleClick(i-1)" :class="{'is-active':currentValue > i-1}"
      ...
      <span class="vux-rater-inner"><span v-html="star" style="margin-top: 5px"></span>
        <span class="vux-rater-outer" :style="{color: activeColor, width: cutPercent + '%'}" v-if="cutPercent > 0 && cutIndex === i-1" v-html="star"></span>
      </span>
    </a>
    <span>{{currStarDesc}}</span>
  </div>
</template>
...
//        this.$emit('curValue', this.currentValue)
        this.currStarDesc = this.starDesc[this.currentValue - 1]
      },
      updateStyle () {
